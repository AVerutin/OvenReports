...
namespace OvenReports.Data
{
    public class QueryRequests
    {
        ...
        public QueryRequests()
        {
            ...
            _chechDt =
                "with mes as " +
                "(SELECT queue_10000.id_posad,queue_10000.id_coil,queue_10000.pos, " +
                ...
                "t.unit_id c_id_coil, t.pos as c_pos, t.date_close::timestamp as date_close, " +
                "pw.value_s::numeric as c_coil_weight,pw.param_date, pn.value_s::numeric as c_coil_num, " +
                ...
                "where t.node_id = 10000 and t.project_id = 1 " +
                "and t.date_close between ''{0}'' and ''{1}'' " +
                ...
                "dt as (select r.unit_id_parent id_posad, l.unit_id as unit_dt, " +
                "t.pos as pos, tm_beg, tm_end, pw.value_s::numeric as billet_weight, pw.param_date as billet_date, " +
                "pt.value_s::numeric coil_weight_dt, pt.param_date coil_date_param, " +
                "coalesce( pp.value_s::numeric,0) as id_prod, p.value_s melt, pc.value_n count, mes.id_coil " +
                "from mts.locations l join mts.units_relations rd on rd.unit_id_child = l.unit_id " +
                "join mts.unit_tasks t on t.unit_id = rd.unit_id_parent and t.node_id = 20100 and t.project_id = 1 " +
                ...
                "join mts.passport p on p.unit_id = r.unit_id_parent and p.project_id = 1 and p.param_id = 10000001 " +
                "join mts.passport pc on pc.unit_id = r.unit_id_parent and pc.project_id = 1 and pc.param_id = 10000004 " +
                ...
                "where l.node_id = 15100) SELECT mes.id_posad, mes.melt||'-'|| mes.pos as eu_mes, mes.coil_weight, date_close, " +
                "dt.melt||'-'||dt.pos as eu_dt, dt.coil_weight_dt, dt.coil_date_param, tm_beg, tm_end, dt.billet_weight, dt.billet_date FROM mes " +
                "left join dt on dt.id_coil = mes.id_coil ORDER BY mes.date_close;";
